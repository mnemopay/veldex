...
exports.loadProfileFromEnv = loadProfileFromEnv;
...
function validateProfile(profile) {
    ...
    if (!profile.fullName || profile.fullName.trim().length < 2)
        errors.push("fullName required (min 2 chars)");
    if (!profile.email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(profile.email))
        errors.push("valid email required");
    if (!profile.shipping)
        errors.push("shipping address required");
    if (profile.shipping) {
        if (!profile.shipping.line1)
            errors.push("shipping.line1 required");
        if (!profile.shipping.city)
            errors.push("shipping.city required");
        if (!profile.shipping.state)
            errors.push("shipping.state required");
        if (!profile.shipping.zip)
            errors.push("shipping.zip required");
