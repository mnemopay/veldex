...
exports.ShopifyCheckoutStrategy = void 0;
class ShopifyCheckoutStrategy {
    name = "shopify";
    canHandle(url) {
        try {
    ...
    // ── Private helpers ──────────────────────────────────────────────────
    async findAddToCartButton(page) {
    ...
    async fillContactInfo(page, profile) {
        // Email field
    ...
    async fillShippingAddress(page, profile) {
        const fieldMap = [
        ...
        for (const [selectors, value] of fieldMap) {
            if (value)
                await this.fillFirstMatch(page, selectors, value);
        }
        // Country selector
        await this.selectFirstMatch(page, ['#shipping-country', 'select[name="countryCode"]', 'select[name="shipping[country]"]'], profile.shipping.country);
        // State/Province selector
        await page.waitForTimeout(500); // wait for state options to load after country
        await this.selectOrFill(page, ['#shipping-province', 'select[name="zone"]', 'select[name="shipping[province]"]', 'input[name="province"]'], profile.shipping.state);
    }
    async continueToShipping(page) {
        const selectors = [
    ...
    async selectCheapestShipping(page) {
    ...
    async fillPayment(page, profile) {
    ...
    // ── Utility helpers ────────────────────────────────────────────────
    async fillFirstMatch(page, selectors, value) {
        for (const sel of selectors) {
            try {
                const field = await page.$(sel);
                if (field && await field.isVisible()) {
    ...
    async selectFirstMatch(page, selectors, value) {
        for (const sel of selectors) {
            try {
    ...
    async selectOrFill(page, selectors, value) {
        for (const sel of selectors) {
            try {
...
exports.ShopifyCheckoutStrategy = ShopifyCheckoutStrategy;
