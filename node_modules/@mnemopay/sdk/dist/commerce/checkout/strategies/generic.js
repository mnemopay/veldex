...
exports.GenericCheckoutStrategy = void 0;
class GenericCheckoutStrategy {
    name = "generic";
    canHandle(_url) {
    ...
    async execute(ctx, profile) {
        ...
        try {
            ...
            if (!atCheckout)
                return this.fail("Could not reach checkout", steps, startTime);
            ...
            if (!submitted)
                return this.fail("Could not find submit/place order button", steps, startTime);
    ...
    async fillCheckoutForm(page, profile) {
        let filled = 0;
        ...
        try {
            ...
            for (const sel of stateSelectors) {
                const select = await page.$(sel);
                if (select && await select.isVisible()) {
                    const tag = await select.evaluate((e) => e.tagName.toLowerCase());
                    if (tag === "select") {
...
exports.GenericCheckoutStrategy = GenericCheckoutStrategy;
