...
exports.default = createSandboxServer;
...
function createAgent() {
    ...
    const persistDir = process.env.MNEMOPAY_PERSIST_DIR ||
...
const TOOLS = [
    ...
    {
        name: "reputation",
        description: "Full reputation report: score, tier, settlement rate, total value settled, " +
            "memory stats. Use to prove agent trustworthiness to other agents or users.",
        inputSchema: { type: "object", properties: {} },
    },
    {
        name: "dispute",
        description: "File a dispute against a settled transaction within the dispute window (24h). " +
    ...
    {
        name: "fraud_stats",
        description: "View fraud detection stats: charges tracked, flagged agents, blocked agents, " +
    ...
    // ── Commerce tools ───────────────────────────────────────────────────
    {
        ...
        description: "Set a shopping mandate — defines what the agent can buy. " +
    ...
    {
        name: "shop_search",
        description: "Search for products within the current shopping mandate. " +
    ...
    {
        name: "shop_buy",
        description: "Purchase a product. Holds funds in escrow — money is NOT released " +
    ...
    },
    {
        ...
        description: "Confirm that a purchased item was delivered. This releases the escrow " +
    ...
    {
        name: "shop_orders",
        description: "List all shopping orders and spending summary. Shows order status, " +
    ...
    {
        name: "payout_create",
        description: "Initiate a bank transfer (payout) via Paystack. First creates a transfer recipient, " +
    ...
    {
        name: "payout_status",
        description: "Check the status of a Paystack transfer/payout by transfer code.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    // ── Webhooks ──────────────────────────────────────────────────────────────
    {
        ...
        description: "Register a webhook URL to receive payment events (charge.success, transfer.success, etc.). " +
...
async function executeTool(agent, name, args) {
    switch (name) {
        ...
        case "settle": {
            ...
            return JSON.stringify({ txId: tx.id, amount: tx.amount, status: tx.status, rail: agent.paymentRail?.name });
        }
        case "refund": {
        ...
        case "dispute": {
            if (!("dispute" in agent))
                throw new Error("Disputes only available in quick mode");
            const d = await agent.dispute(args.txId, args.reason);
            return JSON.stringify({ disputeId: d.id, txId: d.txId, status: d.status, reason: d.reason });
        }
        case "fraud_stats": {
            if (!("fraud" in agent))
                throw new Error("Fraud stats only available in quick mode");
        ...
        case "shop_search": {
            ...
            return results.map((p, i) => `${i + 1}. ${p.title} — $${p.price.toFixed(2)} from ${p.merchant}` +
        ...
        case "shop_orders": {
            ...
            const list = orders.map((o) => `• ${o.product.title} — $${o.product.price.toFixed(2)} [${o.status}]` +
        ...
        case "agent_fico_score": {
            const fico = new index_js_2.AgentCreditScore();
        ...
        case "payout_create": {
            const rail = agent.paymentRail;
            if (!rail || rail.name !== "paystack")
                throw new Error("payout_create requires Paystack rail. Set MNEMOPAY_PAYMENT_RAIL=paystack");
            const recipient = await rail.createTransferRecipient(args.accountName, args.accountNumber, args.bankCode);
            const transfer = await rail.initiateTransfer(recipient.recipientCode, args.amount, args.reason, agent.agentId);
            return JSON.stringify({
                status: "initiated",
        ...
        case "payout_status": {
            const rail = agent.paymentRail;
            if (!rail || rail.name !== "paystack")
                throw new Error("payout_status requires Paystack rail");
...
async function startServer() {
    ...
    const server = new index_js_1.Server({ name: "mnemopay", version: "1.2.0" }, { capabilities: { tools: {}, resources: {}, prompts: {} } });
    // ── Tools ───────────────────────────────────────────────────────────────
    ...
    server.setRequestHandler(types_js_1.CallToolRequestSchema, async (request) => {
        ...
        try {
            ...
            if (requiredKey) {
                const providedKey = request.params?._apiKey || process.env.MNEMOPAY_CLIENT_KEY || "";
                if (!(0, index_js_2.constantTimeEqual)(providedKey, requiredKey)) {
                    return {
    ...
    server.setRequestHandler(types_js_1.ListPromptsRequestSchema, async () => ({
        prompts: [
            ...
            {
                name: "session-start",
                description: "Load memory context at the beginning of a session. " +
            ...
            {
                name: "session-end",
                description: "Consolidate memory and save a session summary before stopping. " +
    ...
    server.setRequestHandler(types_js_1.GetPromptRequestSchema, async (request) => {
        ...
        if (name === "session-end") {
            ...
            const result = await agent.onSessionEnd(summary);
            const profile = await agent.profile();
            return {
                messages: [
                    {
    ...
    if (useHttp) {
        ...
        const rateLimiter = new index_js_2.RateLimiter({
        ...
        app.get("/.well-known/agent.json", (_req, res) => {
            res.json(agent.agentCard(process.env.MNEMOPAY_URL || `http://localhost:${process.env.PORT || 3200}`, process.env.MNEMOPAY_CONTACT));
        });
        ...
        // Tool discovery
        app.get("/api/tools", mcpAuth, (_req, res) => {
            res.json({
        ...
        app.post("/api/:tool", mcpAuth, async (req, res) => {
        ...
        app.post("/api/commerce/mandate", mcpAuth, async (req, res) => {
            try {
        ...
        app.post("/api/commerce/search", mcpAuth, async (req, res) => {
            try {
                const commerce = await getCommerce();
        ...
        app.post("/api/commerce/purchase", mcpAuth, async (req, res) => {
            try {
                const commerce = await getCommerce();
        ...
        app.post("/api/commerce/confirm", mcpAuth, async (req, res) => {
            try {
                const commerce = await getCommerce();
        ...
        app.post("/api/commerce/cancel", mcpAuth, async (req, res) => {
            try {
                const commerce = await getCommerce();
        ...
        app.get("/api/commerce/orders", mcpAuth, async (req, res) => {
            try {
                const commerce = await getCommerce();
