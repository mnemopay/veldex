...
class PaystackRail {
    ...
    processedRefs = new Map(); // ref → timestamp
    /** In-flight captures to prevent concurrent verify races */
    inFlightCaptures = new Map();
    static MAX_PROCESSED_REFS = 10_000;
    static REF_TTL_MS = 24 * 60 * 60_000; // 24h
    ...
    async createHold(amount, reason, agentId, options) {
        ...
        const reference = options?.metadata?.reference ||
     ...
     */
    async initiateTransfer(recipientCode, amount, reason, agentId, idempotencyKey) {
        if (!recipientCode)
    ...
    _evictExpiredRefs() {
        if (this.processedRefs.size <= PaystackRail.MAX_PROCESSED_REFS / 2)
            return;
