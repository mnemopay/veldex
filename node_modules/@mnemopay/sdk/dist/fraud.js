...
class ReplayDetector {
    ...
    check(agentId, amount, reason, counterpartyId) {
        const raw = `${agentId}|${amount}|${reason}|${counterpartyId ?? ""}`;
        const fp = require("crypto").createHash("sha256").update(raw).digest("hex");
        const now = Date.now();
...
class FraudGuard {
    ...
    constructor(config) {
        ...
        if (config && typeof config === "object") {
            const validKeys = new Set(Object.keys(exports.DEFAULT_FRAUD_CONFIG));
            const unknown = Object.keys(config).filter(k => !validKeys.has(k));
            if (unknown.length > 0) {
                // eslint-disable-next-line no-console
                console.warn(`[mnemopay] FraudGuard: ignoring unknown config keys: ${unknown.join(", ")}. ` +
                    `Valid keys: ${Array.from(validKeys).join(", ")}`);
            }
        }
        this.config = { ...exports.DEFAULT_FRAUD_CONFIG, ...config };
        // Deep merge geo config
        this.config.geo = { ...exports.DEFAULT_FRAUD_CONFIG.geo, ...config?.geo };
     ...
     */
    assessCharge(agentId, amount, reputation, accountCreatedAt, pendingCount, ctx, reason, counterpartyId) {
        const signals = [];
        ...
        if (ring) {
            ...
            for (let i = 0; i < ring.count; i++) {
                ...
                const age = now - ts;
                if (age >= 86_400_000)
                    break; // ring is append-ordered newest-at-head; older won't match
                chargesLastDay++;
        ...
        if (ring) {
            ...
            for (let i = 0; i < ring.count && recentAmounts.length < 20; i++) {
                const pos = ((ring.head - 1 - i) % cap + cap) % cap;
                if (now - ring.buf[pos * 2] >= 3_600_000)
                    break;
    ...
    recordCharge(agentId, amount, ctx) {
        ...
        if (this.isolationForest) {
            ...
            const recent10Amounts = [];
            if (ring) {
                const cap = ring.capacity;
                for (let i = 0; i < ring.count; i++) {
                    const pos = ((ring.head - 1 - i) % cap + cap) % cap;
                    if (now - ring.buf[pos * 2] >= 600_000)
                        break;
                    recent10Count++;
    ...
    applyPlatformFee(txId, agentId, grossAmount) {
        if (!Number.isFinite(grossAmount) || grossAmount <= 0)
            throw new Error("Gross amount must be a positive finite number");
        ...
        if (this.feeLedger.length > 200)
            this.feeLedger.splice(0, this.feeLedger.length - 200);
    ...
    assessGeo(agentId, ctx) {
        ...
        const dampen = (w) => Math.round(w * (1 - trust * 0.5) * 100) / 100;
     ...
     */
    updateGeoProfile(agentId, ctx) {
        if (!ctx?.country || !this.config.geo.enabled)
            return;
        const profile = this.getOrCreateGeoProfile(agentId);
        const now = Date.now();
    ...
    static deserialize(json, config) {
        ...
        try {
            ...
            if (data.chargeHistory) {
                for (const [agentId, entries] of data.chargeHistory) {
                    const cap = 5_000;
                    const ring = { buf: new Float64Array(cap * 2), head: 0, count: 0, capacity: cap };
            ...
            if (data.geoProfiles) {
                guard.geoProfiles = new Map(data.geoProfiles.map(([k, v]) => [
                    k,
                    {
                        ...v,
                        ...
                        countryChanges: Array.isArray(v.countryChanges)
                            ? v.countryChanges.filter((t) => typeof t === "number" && Number.isFinite(t))
                            : [],
                        countryCounts: v.countryCounts && typeof v.countryCounts === "object"
                            ? Object.fromEntries(Object.entries(v.countryCounts).filter(([, c]) => typeof c === "number" && Number.isFinite(c) && c >= 0))
                            : {},
                    },
                ]));
            }
