...
class Ledger {
    ...
    constructor(existingEntries) {
        ...
        if (hasSummary) {
            ...
            if (last.accountBalances) {
                for (const [k, v] of Object.entries(last.accountBalances)) {
                    this._accountBalances.set(k, v);
                }
            }
            existingEntries = existingEntries.slice(0, -1);
        }
        const validated = existingEntries.map(e => {
            const debit = Number(e.debit);
            const credit = Number(e.credit);
            if (!Number.isFinite(debit) || debit < 0)
                throw new Error(`Ledger entry ${e.id}: debit must be a non-negative finite number`);
            if (!Number.isFinite(credit) || credit < 0)
                throw new Error(`Ledger entry ${e.id}: credit must be a non-negative finite number`);
            if (!Number.isFinite(e.seq) || e.seq < 0)
                throw new Error(`Ledger entry ${e.id}: seq must be a non-negative number`);
    ...
    verifyChain() {
        ...
        for (let i = 0; i < this.entries.length; i++) {
            ...
            if (entry.prevEntryHash === undefined)
                continue;
            chainedCount++;
            if (i === 0) {
